...
class TransactionService {
  ...
  // Hàm helper lấy nhanh đường dẫn tới sub-collection transactions của user hiện tại
  CollectionReference _getUserTransactionsRef() {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) throw Exception('Chưa đăng nhập');
    // Truy cập theo cấu trúc lồng: users -> {uid} -> transactions
  ...
  Future<void> addTransaction(TransactionModel t) async {
    ...
    print('✅ Đã thêm transaction vào sub-collection: ${data['amount']} - ${data['category']}');
  }
  ...
  Future<void> updateTransaction(TransactionModel t) async {
    ...
    if (docSnapshot.exists && docData != null && docData['uid'] != FirebaseAuth.instance.currentUser?.uid) {
      throw Exception('Không có quyền sửa');
    }
  ...
  Stream<List<TransactionModel>> getTransactions() {
    ...
    print('🔍 Đang query sub-collection transactions cho uid: ${user.uid}');

    return _getUserTransactionsRef()
        .orderBy('date', descending: true)
        .snapshots()
        .map((snapshot) {
      print('📊 Tìm thấy ${snapshot.docs.length} giao dịch trong sub-collection');
